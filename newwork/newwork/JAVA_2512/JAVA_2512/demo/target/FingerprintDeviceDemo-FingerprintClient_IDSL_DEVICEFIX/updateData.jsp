		...
		<div class="row">
			<div class="col-md-8 col-md-offset-2">
				<div class="sync-card">
					...
					<div class="sync-subtitle">Selected date ke basis par manual recovery sync. Status ke liye UPD_DATE aur delete ke liye DEL_DATE use hoga.</div>
...
<script type="text/javascript">
	...
	function buildDeleteText(ext){
		return "Delete sync done | date=" + (ext.syncDate || "")
			+ ", devices=" + (ext.devices || 0)
			+ ", onlineDevices=" + (ext.onlineDevices || 0)
	...
	$(function(){
		...
		$("#syncStatusByDateBtn").click(function(){
			...
			setResult($("#syncStatusResult"), true, "Running status sync for " + syncDate + " ...");
			$.ajax({
				...
				success: function(result){
					...
					setResult($("#syncStatusResult"), result && result.code === 100, text + ((result && result.code !== 100 && ext.error) ? " | error=" + ext.error : ""));
				},
				error: function(){
					setResult($("#syncStatusResult"), false, "Status sync failed for " + syncDate + ".");
				}
			});
		});
		...
		$("#syncDeleteByDateBtn").click(function(){
			...
			setResult($("#syncDeleteResult"), true, "Running delete sync for " + syncDate + " ...");
			$.ajax({
				...
				success: function(result){
					...
					setResult($("#syncDeleteResult"), result && result.code === 100, text + ((result && result.code !== 100 && ext.error) ? " | error=" + ext.error : ""));
				},
				error: function(){
					setResult($("#syncDeleteResult"), false, "Delete sync failed for " + syncDate + ".");
				}
			});
		});
	});
