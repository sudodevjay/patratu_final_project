param(
    [string]$AppName = "FingerprintClient_IDSL_DEVICEFIX",
...
$DistRoot = Join-Path $ProjectRoot "dist\\client-temp"
$PackageDir = Join-Path $DistRoot $AppName
$ZipPath = Join-Path $DistRoot ($AppName + ".zip")
$LegacyBuildDir = Join-Path $ProjectRoot ("dist\client-build-" + $AppName)

$LauncherBuildRoot = Join-Path $ProjectRoot "packaging\client-launcher\build"
...
$ExplodedWarDir = Join-Path $ProjectRoot "demo\target\FingerprintDeviceDemo-0.0.1-SNAPSHOT"
$WarOutput = Join-Path $ProjectRoot "demo\target\FingerprintDeviceDemo-0.0.1-SNAPSHOT-IDSL-DEVICEFIX.war"
...
if (Test-Path $LegacyBuildDir) {
    Remove-Item -Path $LegacyBuildDir -Recurse -Force
}
Get-ChildItem -Path $DistRoot -File -Filter ($AppName + "-*.zip") -ErrorAction SilentlyContinue | Remove-Item -Force
...
& $JarExe -cf "..\FingerprintDeviceDemo-0.0.1-SNAPSHOT-IDSL-DEVICEFIX.war" .
...
set "JAVA_OPTS=%JAVA_OPTS% -Didsl.photo.db.write.enabled=false"
